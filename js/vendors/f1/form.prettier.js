...
(function (window, document) {
  ...
  function clearErrors(frmObj, errorsSelector) {
    ...
    errorsSelector = errorsSelector || 
      '.' + frmObj.errorsClass.replace(' ', '.');
    frmObj.elm
      .querySelectorAll(errorsSelector)
      .forEach((elMsgs) => elMsgs.parentElement.removeChild(elMsgs));
  }
  ...
  const Form = function (options) {
    ...
    this.getFields();
    if (this.initialValues) this.init(this.initialValues);
  };
  ...
  Form.Error.prototype.focus = function () {
    this.field.focus();
  };
  ...
  Form.FieldType.prototype = {
    ...
    getLabel: function () {
      ...
      if (!label) {
        ...
        if (elLbl) {
          label = elLbl.innerHTML;
        } else label = this.getName();
      }
      return label || 'Field';
    },
    getName: function () {
    ...
    setValue: function (val) {
      this.input.type === 'checkbox'
        ? (this.input.checked = this.input.value === val)
        : (this.input.value = val || '');
    },
    getRequired: function () {
      return this.elm.classList.contains('required');
    },
    ...
    validate: function (options) {
      ...
      if (!valid)
        this.errors.push(
          new Form.Error(this, validator.getInvalidMessage(this, options))
        );
      return valid;
    },
  ...
  Form.Validator.prototype = {
    ...
    getInvalidMessage: function (field, args) {
  ...
  Form.prototype = {
    ...
    showErrors: function () {
      ...
      elMsgs.innerHTML = errors
        .map((e) => '<div class="error">' + e.message + '</div>')
        .join('');
      this.elm.appendChild(elMsgs);
    },
    clearErrors: function () {
      clearErrors(this, '.' + this.summaryClass);
    ...
    init: function (initialValues, nameSpace) {
      ...
      this.fields.forEach((f) => {
        ...
        const fid = nameSpace
          ? fname.replace(`${nameSpace}[`, '').replace(']', '')
          : fname;
        if (initialValues[fid]) {
          f.setValue(initialValues[fid]);
        }
